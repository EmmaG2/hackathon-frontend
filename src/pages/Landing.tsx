...
const CARACTERISTICAS: Caracteristica[] = [
  {
    ...
    descripcion:
      'Ve exactamente cuántas mesas hay libres ahora mismo en cada restaurante. Sin sorpresas al llegar.',
  },
  {
    Icono: CalendarDays,
    titulo: 'Reserva en segundos',
    descripcion:
      'Elige fecha, hora y mesa directamente desde tu teléfono. Sin llamadas, sin formularios largos.',
  },
  {
    Icono: CheckCircle,
    titulo: 'Cancela sin cargo',
    descripcion:
      'Cambia de planes sin penalización hasta 2 horas antes. Tu agenda, tus reglas.',
  },
]

function NavegacionLanding({
  onRestaurante,
}: {
  onRestaurante: () => void
}) {
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 md:px-12 h-16 bg-crema/96 backdrop-blur-sm border-b border-cafe/7">
      ...
      <button
        ...
        className="font-body text-[13px] text-cafe-atenuado hover:text-cafe transition-colors"
      >
        ¿Eres un restaurante?
      </button>
    </nav>
  ...
  return (
    <div className="bg-white/6 border border-white/8 rounded-2xl px-4 py-3.5 flex items-center gap-3 hover:bg-white/10 transition-colors cursor-pointer">
      ...
      <div className="flex-1 min-w-0">
        <p className="font-body font-semibold text-[13px] text-crema-luz truncate">
          {r.nombre}
        </p>

        <p className="font-body text-[11px] text-crema-luz/45">
          {r.zona} · {r.mesas}{' '}
          {r.mesas === 1 ? 'mesa libre' : 'mesas libres'}
        </p>
      </div>

      <div className="flex items-center gap-1.5 shrink-0">
        <span className="w-1.5 h-1.5 bg-disponible rounded-full" />
        <span className="font-body text-[10px] text-disponible">
          Ahora
        </span>
      </div>
    </div>
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          backgroundImage:
            'radial-gradient(circle, #e8dcc8 1.5px, transparent 1.5px)',
          backgroundSize: '28px 28px',
        }}
      />

      <div className="absolute top-16 right-8 bg-amarillo rounded-[18px] px-4 py-3 shadow-lg">
        <p className="font-body text-[10px] font-medium text-cafe/70 uppercase tracking-wider">
          Ahora mismo
        </p>

        <p className="font-display text-[32px] text-cafe leading-none">
          247
        </p>

        <p className="font-body text-[11px] text-cafe/70">
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {CARACTERISTICAS.map((c) => (
            ...
            >
              ...
              <h3 className="font-display text-[18px] text-cafe">
                {c.titulo}
              </h3>

              <p className="font-body text-[13px] text-cafe-atenuado leading-relaxed">
                {c.descripcion}
              </p>
            </div>
          ))}
...
export function Landing() {
  ...
  function irALoginRestaurante() {
    navegar('/login-restaurante')
  }
  ...
  return (
    <div className="min-h-screen bg-crema overflow-x-hidden">
      ...
      <NavegacionLanding
        onRestaurante={irALoginRestaurante}
      />
        ...
        <div className="flex-1 flex flex-col justify-center px-6 md:px-16 py-16 md:py-0">
          ...
          <div className="max-w-lg">
            ...
            <h1 className="font-display text-[44px] md:text-[58px] text-cafe leading-[1.05] mb-5">
              ...
              <span className="text-amarillo-oscuro">
                a un toque
              </span>{' '}
              de distancia.
            </h1>
            ...
            <div className="flex items-center gap-6 mt-10">
              {[
                ...
                >
                  {i > 0 && (
                    <div className="w-px h-8 bg-cafe/10" />
                  )}

                  <div className="flex flex-col">
                    <span className="font-display text-[24px] text-cafe leading-tight">
                      {stat.valor}
                    </span>
      ...
      <SeccionCTAFinal
        onIniciarSesion={iniciarSesionGoogle}
        onRestaurante={irALoginRestaurante}
      />
