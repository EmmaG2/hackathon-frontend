    ...
    <div className="min-h-screen bg-crema pb-20.5 md:pb-12">
      ...
      <div className="max-w-7xl mx-auto px-4.5 md:px-8">
        <div className="pt-14.5 pb-3.5 md:pt-8 md:pb-5 flex flex-col gap-1.5">
          ...
          <h1 className="font-display text-[30px] md:text-[40px] text-cafe">Hola, Renata</h1>
        ...
        <div className="mb-6">
          <button
            onClick={() => navegar('/restaurante/casa-paloma')}
          ...
          >
            ...
            <div className="absolute top-3.5 right-3.5">
              <InsigniaOcupacion porcentaje={70} etiqueta="70% · Ocupado" />
            </div>

            <div className="absolute bottom-4.5 left-4.5 flex flex-col gap-1.5">
              <span className="font-body font-medium text-[10px] text-crema-luz/72">MEXICANA CONTEMPORÁNEA</span>
              <h2 className="font-display text-[28px] md:text-[40px] text-crema-luz">Casa Paloma</h2>
              <span className="font-body text-[12px] md:text-[15px] text-crema-luz/78">⭐ 4.8 · 📍 Roma Norte · $$$</span>
            </div>
          </button>
        </div>

        <div>
          <h2 className="font-display text-[18px] md:text-[24px] text-cafe mb-3">Cerca de ti</h2>
          <div className="flex flex-col gap-3 md:grid md:grid-cols-3 md:gap-4">
            {RESTAURANTES_CERCANOS.map((restaurante) => (
              <TarjetaRestaurante
                key={restaurante.id}
                {...restaurante}
                onClick={() => navegar(`/restaurante/${restaurante.id}`)}
              />
            ))}
          </div>
        </div>
